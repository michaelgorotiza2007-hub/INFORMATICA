...
function goToInterface(viewId, param = null) {
    ...
    setTimeout(() => {
        ...
        if(viewId === 7) renderAwardMain(); 
        if(viewId === 8) renderAwardsDetail();
        if(viewId === 9) renderInfo(); 
...
function extractVideoID(url) {
    if(!url) return null;
    // Regex robusta para detectar ID de cualquier link youtube
...
function renderAwardMain() {
    ...
    db.collection("settings").doc("video_"+state.selectedCourse).get().then(doc => {
        ...
        if(doc.exists && doc.data().url) {
            ...
            if(id) box.innerHTML = `<iframe width="100%" height="100%" src="https://www.youtube.com/embed/${id}" frameborder="0" allowfullscreen></iframe>`;
            else box.innerHTML = '<p style="color:#666; padding-top:200px;">Error enlace video</p>';
        } else {
...
function saveCategory() { db.collection("award_cats").add({name:val('cat-name'), cover:val('cat-cover'), courseId:state.selectedCourse}).then(()=>{closeModal('category-modal'); renderAwardMain();}); }
function saveNominee() { db.collection("awards").add({name:val('nom-name'), photo:val('nom-photo'), cert:val('nom-cert'), type:val('nom-type'), catId:state.currentAwardCat}).then(()=>{closeModal('nominee-modal'); renderAwardsDetail();}); }